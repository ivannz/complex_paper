...
Bayesian inference in not safeguarded against overfitting and biases: there are quite a handful
...
and prior come from a conjugate families, making the posterior explicit, the questions of interest
...
distribution approximation family.
...
\textbf{Monte Carlo} approximations. This key idea of the former is to seek an approximation
...
\tag{master-identity}
\end{align}
Instead of minimizing the divergence of the approximation from the posterior, we maximize the
\textbf{Evidence Lower Bound} with respect to $q(\omega)$:
$$
q^* \in
  \arg\max_{q\in Q}
    \mathcal{L}(q)
...
\subsubsection{Gradient of the KL-divergence in $\real$ case} % (fold)
...
% subsubsection gradient_of_the_kl_divergence_in_R_case (end)
