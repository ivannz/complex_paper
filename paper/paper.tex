...
$, or zero otherwise. Figure~\ref{fig:hist__and__threshold__tradeoff} (top) displays
the general shape of the distribution of $\log \alpha$ on the Fashion-MNIST dataset in
sec.~\ref{sub:mnist_like_datasets}. There always is a single mode within the moderately
negative range, and one or two modes to the right of zero, and the trough between the
peaks of the negative mode and the first positive one is around zero. The relative mass
...
plot and the results of other experiments, it can be concluded that $C$ has much more
substantial impact on the sparsity and performance, than the choice of $\tau$. Furthermore,
for $\tau > 0$ the performance quickly saturates and while compression continues to
drop.
...
concentrated around $\mu$, which is used as a stand-in value for $W$, when the network
is used in inference mode. Since $W$ is $\real$- or a proper $\cplx$-Gaussian, the random
variable $
...
to exclude noisy parameters and encourage higher sparsity, at the same time staying aware
...
respectively.
%
Plots~\ref{fig:mnist-like__trade-off} depict performance-compression curve on MNIST-like
datasets for the studied models and methods from sec.~\ref{ssub:variational_dropout} (VD)
and~\ref{ssub:empirical_bayes} (ARD). Besides that we tried \texttt{raw} or \texttt{fft}
input features, varied $
  C \in \{\tfrac3{2^{k+1}}\colon k=1..16\}
$, and repeated the experiment $4$ times.

% observation: raw features -- alright

fft features with wide dense network exhibit improving performance with the compression rate
...
\begin{figure}[!ht]
  ...
  \end{subfigure}
  \caption{%
    Performance-compression frontier for the deep network of \citet{trabelsi_deep_2017}.
    Shaded horizontal region represents the $\mu\pm\sigma$ bands for the performance of the
    uncompressed model. \textbf{\color{blue} VD} and \textbf{\color{orange} ARD} denote
    methods \ref{ssub:variational_dropout}) ans \ref{ssub:empirical_bayes}, respectively.
    The \textbf{\color{pink} version} of the network with kernel $3$ in every convolution compressed with VD
    is (\ref{ssub:variational_dropout}).
  }
