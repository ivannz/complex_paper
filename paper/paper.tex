...
$, respectively. $\cplx$-Gaussians are closed under affine transformations: if $y = A z + b$
...
$C = 0$. From \eqref{eq:c-gauss-entropy-derivation} the entropy of a proper $\cplx$-Gaussain is
...
W x + b
  ...
  \,. $$
Structure of the matrices, involved in this expression, imply
\begin{align*}
  (I \otimes x^\top) \diag{\vec{\Sigma}} (I \otimes x^\top)^\hop
    ...
    \,.
\end{align*}
In a similar fashion we obtain the expression for the relation matrix of
the transformation:
\begin{equation*}
