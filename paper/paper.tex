...
\begin{equation}  \label{eq:elbo-general-kl-div}
  KL(q \| \pi)
    ...
    \,.
\end{equation}
We omit subscripts ${ij}$ for brevity in the next sectons.
...
as the real part of the $\cplx$ output.
...
%
\todo{reword}
We observe dual effect of ``fine-tune'' stage: it improves performance of highly compressed
models since they are more regularized (by poking holes) and thus serves as further fitting.
At the same time, undercompressed models, behave as if they were over parameterized
and the fine tune stage overfits.
...
the $q_\theta$ factorizes into univariate distributions, whether a $\cplx$ or $\real$ parameter
is zero or not is determined from $\alpha = \tfrac{\sigma^2}{\lvert\mu\rvert^2}$: non-zero
if $\log \alpha \leq \tau$, zero otherwise. Figure~\ref{fig:hist__and__threshold__tradeoff}
(top) displays the general shape of the distribution of $\log \alpha$ on the Fashion-MNIST
dataset in sec.~\ref{sub:mnist_like_datasets}. This shape appears to be typical in every
experiment conducted in this study: there always is a single mode within the moderately
negative range, and one or two modes to the right of zero. In every experiment the trough
between the peaks of the negative mode and the first positive one is around zero. The
relative mass of each mode changes with $C$ and the higher the coefficient is, the more
mass is shifted to the right.
%
The bottom graph on figure~\ref{fig:hist__and__threshold__tradeoff} is constructed for
$C \in \{\tfrac1{20}, \frac1{200}\}$ before running the ``fine-tune'' stage. From the
relative positions of the compression curves on this plot and from the results of other
experiments we conclude that $C$ has much more substantial impact on the sparsity and
performance, than the choice of $\tau$. Furthermore, for $\tau > 0$ the performance quickly
saturates and while compression continues to drop.
...
\begin{figure}[ht]
  ...
  \caption{%
    ...
    (\textit{bottom}) Test split performance and compression rate for $\cplx$-valued net
    by \citet{trabelsi_deep_2017} on MusicNet.
  }
...
approximate posterior $q_\theta(W)$. Since $W$ is $\real$- or a proper $\cplx$-Gaussian,
the random variable $
...
at least $90\%$ chance of a non-zero $\real$/$\cplx$ parameter. In our opinion, $\tau = 3$
\citep{molchanov_variational_2017,kingma_variational_2015} retains parameters with randomness
that significantly exceeds the magnitude of the mean, although $\mu$ is typically used as
a stand-in for $W$ when the network runs in inference mode. Given that $q_\theta$ is merely
an approximation, we pick $\tau = -\tfrac12$ to exclude noisy parameters and encourage
higher sparsity.
...
has incompressible parameters, such as layer biases, shift and scaling from $\real$-and
...
We conduct a moderately sized experiment on MNIST and similar $28\times 28$ greyscale
image datasets to study the compression-performance trade-off of the proposed $\cplx$-variational
...
The values of $n_\cdot$ depend on the input features and are summarized in table~\ref{tbl:mnist-like-model-spec}.
...
First we compare the $\real$-valued networks against their $\cplx$-valued counterparts with
approximately half as many parameters as suggested by \citet{monning_evaluation_2018}. In
this experiment stages last for $40$, $75$ and $40$ epochs, respectively, and the base learning
rate is scaled after $5$, $10$, and $20$-th epoch by $\tfrac1{10}$, $\tfrac1{20}$ and $\tfrac1{100}$,
respectively.

% discuss the plots describing how the test-performance grows with compression and then drops if the model is too sparse
Plots on figure~\ref{fig:mnist-like__trade-off} show the results on the ``MNIST'' dataset for
$\tau\in\{-\tfrac12, \tfrac32\}$ and $
...
is too harsh for sparsification tasks \citep{kharitonov_variational_2018}.

Nevertheless, it is clear form the results depicted in \ref{where}, that VD and ARD
methods yield very similar compression-performance frontiers.

% second experiment: use scheduler Trabelsi et al. (2018) and show them plotz!
In the second experiment we use a much slower learning rate scheduler and study only the 
VD method.
...
\begin{figure}[!ht]
  ...
  \caption{%
    ...
    (top) and \textit{SimpleConvModel} (bottom) for methods in sections~\ref{ssub:variational_dropout}
    and~\ref{ssub:empirical_bayes}.
  }
...
that we study VGG16 model \citep{simonyan_very_2015} and its $\cplx$-counterpart. Each
network is trained stagewise for $20$, $40$, and $20$ epochs on the full training split
of $50k$ colour $32\times32$ images. We use only raw colour features (\texttt{raw}), since
the results of the experiments on MNIST left us sceptical about the utility of the Fourier
features for image recognition. We use fast learning rate scheduler from sec.~\ref{sub:mnist_like_datasets}.
...
replaced by their $\cplx$-valued versions. To maintain comparability, we halve the
filter sizes of $\cplx$ model \citep{monning_evaluation_2018}. 
...
\begin{figure}[!ht]
  ...
  \begin{subfigure}[b]{0.9\textwidth}  % imcl2019-style swears at this
    \centering
    \includegraphics[draft,width=\linewidth]{../assets/figure__cifar10__trade-off/musicnetram__fft__-0.5.pdf}
  \end{subfigure} \\%
  % \hfill
  \begin{subfigure}[b]{0.9\textwidth}  % imcl2019-style swears at this
    \centering
    \includegraphics[draft,width=\linewidth]{../assets/figure__cifar10__threshold/cifar10__threshold.pdf}
  \end{subfigure}
  \caption{%
    Pre ``fine-tune'' performance of the best models from figure \ref{fig:musicnet_trade-off}
...
The halved $\cplx$ network significantly underperforms in comparison with its $\real$ counterpart
...
We use training from sec.~\ref{sub:fitting_and_sparsification} with epochs lasting for $12$,
...
epoch, used by \citet{trabelsi_deep_2017}. Besides stagewise training and shorter ``dense''
...
\begin{itemize}
  ...
  \item faster learning rate reduction schedule (sec.~\ref{sub:fitting_and_sparsification})
\end{itemize}
...
\begin{figure}[!ht]
  ...
  \caption{%
    ...
    uncompressed model.
    \textit{top}: Original model ($k=6$) compressed with \textbf{\color{blue} VD}
    (\ref{ssub:variational_dropout}) and \textbf{\color{orange} ARD} (\ref{ssub:empirical_bayes});
