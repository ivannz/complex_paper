...
In this section we compare the complex variational dropout techniques discussed above
...
\begin{itemize}
  ...
  \item global gradient clipping to within an $\ell_2$-norm ball of radius $0.05$
\end{itemize}
...
\begin{figure}[!ht]
  ...
  \begin{subfigure}[b]{0.9\textwidth}
    \centering
    \includegraphics[width=1.\linewidth,draft]{../notebooks/assets/grad_log.pdf}
  ...
  \caption{%
    ...
    \textit{bottom}: The probability of 
  }
...
$-2.5$ yield a reasonably high chance of a non-zero parameter. Given that $q_\theta$ is
merely an approximation we opt for a slightly higher $\log \alpha$ threshold, and thus
set $\tau = -\tfrac12$ in every experiment, despite it being lower than the recommended
$\tau = 3$ in \citep{molchanov_variational_2017}, but yields much higher levels of sparsity.
The rationale behind our choice was twofold: we use an extra fine tuning stage, and keep the
parameters that are away from zero with high probability.
...
We conduct a moderately sized numerical experiment on MNIST and similar datasets to see
whether the proposed $\cplx$-var dropout indeed reasonably sparsifies complex-valued networks.

We compare with the studied $\cplx$-valued networks against their classical $\real$-valued
counterparts both in terms of the resulting sparsity and test performance.

Although image data is not $\cplx$ in nature, we apply two-dimensional Fourier Transform
in order to get $\cplx$-valued inputs. We concede that this is a rather unnatural representation
of image data, however the selection of publicly available datasets, which are not artificially
$\cplx$-valued, is limited to audio, speech and music. Indeed, as is shown by the numerical
experiments, a real-valued model is trained on the Fourier transformed data underperforms
relative to its architecturally similar twin fit to on the raw color data.

\citet{trabelsi_deep_2017} study the applicability of $\cplx$-valued networks to image
classification (CIFAR-10, CIFAR-100, and a subset of SVHN). Their approach is to either
embed $\real$ in $\cplx$ (by $\Re z = 0$), or train a smaller auxiliary network, that
synthesizes the imaginary component from the raw channel data. \todo{DO THIS}
In our experiments we opt to transforming the input $2d$ data with 
