...
stages: ``dense'' $\to$ ``sparsify'' $\to$ ``fine-tune''.
%
The last two stages are used to compare the .

At the first, ``dense'', stage every network retains its original non-bayesian layers
and is fit ``as-is'' using only the likelihood from \eqref{eq:elbo} without variational
dropout.

During the ``sparsify'' stage we make the relevant layers bayesian and apply $\cplx$-variational
...
Before transitioning to the ``fine-tune'' stage we compute the mask of non-zero weights
in each layer based on the relevance scores $\alpha$ (sec.~\ref{sub:dropout}). Since
the variational approximation factorizes into univariate distributions, whether a $\cplx$
or $\real$ parameter is zero or not is determined from its relevance score $
  \alpha = \tfrac{\sigma^2}{\lvert\mu\rvert^2}
$: non-zero (relevant) if $\log \alpha \leq \tau$, zero (irrelevant) otherwise. Figure~%
\ref{fig:typical_log_alpha_distrib} depicts the distribution of $\log \alpha$ values
from \textbf{some run}, which exhibits the typical behaviour we noticed in all experiments
conducted: there always is a single mode within the moderately negative range, and one or
two modes to the right of $0$. The relative mass of each mode changes with $C$ and the
higher the coefficient is, the more mass is shifted to the positive half. However, in every
exepriment the trough between the peaks of the negative mode and the first positive one
is around zero.

Thus, in every experiment we set $\tau = -\tfrac12$, which is admittedly very low, especially
considering that the recommended setting in \citep{molchanov_variational_2017} is $3$. This
yields high levels of sparsity,
The rationale behind our choice was twofold: we use an extra fine tuning
stage, and keep the weights that are away from zero with high probability.

These masks are then used during ``fine-tune'' to train the remaining non-zero parameters
similar to the ``dense'' stage, i.e. with layers non-bayesian layers, but with fixed
sparsity masks.

Unless specified otherwise we use ADAM optimizer with the base learning rate of $10^{-3}$
and the annealing schedule from .
