...
During the second stage ``sparsify'' we make the relevant layers bayesian and apply
variational dropout sec.~\ref{ssub:variational_dropout} (or sec.~\ref{ssub:empirical_bayes})
or their $\real$ counterparts. To be able to study the attainable sparsity levels we
inject a coefficient $C \in (0, 1]$ at the KL divergence term in
\eqref{eq:elbo}:
\begin{equation}  \label{eq:elbo_with_coef}
    ...
    \,. \tag{\ref{eq:elbo}'}
\end{equation}
Increasing $C$ yields increasingly more sparse networks upon termination of this stage.
...
$: non-zero (relevant) if $\log \alpha \leq \tau$, zero (irrelevant) otherwise.
In every experiment we set $\tau = -\tfrac12$, which yields extreme levels of sparsity,
especially considering that the recommended setting in \citep{molchanov_variational_2017}
was $3$. The rationale behind our choice was twofold: we use an extra fine tuning
