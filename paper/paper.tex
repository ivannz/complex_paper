...
technique \cite{molchanov_variational_2017}. In \cite{kingma_variational_2015} the Binary (dropconnect) and
Gaussian dropout \cite{hinton,dropconnect,srivasathva} were framed as variational inference
by treating them as a factorized posterior approximations of the form $
...
Focusing on Gaussian dropout, \cite{kingma_variational_2015} proposed Variational Dropout technique
for Bayesian model regularization: $\alpha$ is treated as a variational parameter, that enabled
its optimization through SGVB. The variational approximation is a fully factorized
Gaussian distribution with components $
  q_{\theta}(\omega_j)
...
sparsification was voiced in \cite{state-of-sparsity}, where it was show that it yielded
inconsistent results in text translation tasks and systematically underperformed simpler
techniques such as norm pruning and $\ell_0$-regularization, \cite{ell-0-reg}. Despite this
in the experimental section of this paper we consolidate evidence in support of Variational
dropout on various complex-valued acoustic and signal datasets. The most likely explanation
of the ``varying mileage'' is that different tasks benefit from different approaches.
...
in terms of special functions. We point out that unlike the real-valued case the gradients
for the penalty have closed tractable form.
