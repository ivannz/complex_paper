...
\end{equation}
We propse settng $\beta = 2$ for $\cplx$ variational dropout, to make the divergence
term depend only on $\alpha$.
% For $\beta = 2$ we add $\gamma$ to make the right-hand side nonegative. (proof missing)
%
...
\subsection{Stagewise fitting and compression} % (fold)
...
$: non-zero if $\log \alpha \leq \tau$, zero otherwise. Figure~\ref{fig:hist__and__threshold__tradeoff}
...
is around zero.
%
The bottom plot on figure~\ref{fig:hist__and__threshold__tradeoff} is constructed without
running the ``fine-tune'' stage for $C=\tfrac1{2000}$, $\tfrac1{20}$, and $\frac1{200}$.
The latter setting yields one of the better performing models on the validation split in
sec.~\ref{sub:musicnet}. We can observe that by choosing $\tau > 0$ we end up in the
saturation region of the performance and gain only marginal improvement, while losing
in compression. Relative position compression curves shows that $C$ has much more substantial
impact on the target sparsity and performance, than the choice of $\tau$.
...
$-2.5$ yield a relatively high chance of a non-zero parameter. Setting $\tau = 3$
\citep{molchanov_variational_2017,kingma_variational_2015} is, in our opinion, detrimental
since it retains parameters $W \sim q_\theta$, standard deviation of which significantly
exceeds the magnitude of $\mu$, which is typically used as a stand-in for $W$ when the
network is run in inference mode. Given that $q_\theta$ is merely an approximation,
we pick $\tau = -\tfrac12$ as the sparsification threshold for $\log \alpha$ in our
experiments.

% subsection fitting_and_compression (end)

\subsection{Metrics and performance} % (fold)
...
% subsection metrics_and_performance (end)
...
% discussion of TwoLayerDenseModel (TLDM) and SimpleConvModel (SCM)
 the following :
To compactly describe the $\real$/$\cplx$ networks we study in this experiment we use
the following notation: $c(n, k, s)$ is a convolution with $n$ output channels, kernel
width $k$ and stride $s$, and $d(n)$ is a dense layer with $n$ outputs. The values of
$n_\cdot$ depend on the input features and are summarized in table~\ref{tbl:mnist-like-model-spec}.
\begin{itemize}
