...
KL(q(W) \| \pi(W))
  ...
  \,.
\end{equation}
In the derivations in the sections to come we shall omit the subscripts for brevity.
...
function. Note that the limiting value of \eqref{eq:log-moment-for-chi-2} as $
  \lvert \theta \rvert \longleftarrow 0
$ is $- \gamma$, $\gamma$ denotes Euler's constant. Therefore after cancellation the expression
...
expression for a derivative: $x \mapsto \tfrac{e^x}{x}$ for $x < 0$. This observation enables
...
term itself. Otherwise we have to offload the values from the device to cpu, compute the special
...
parameters are independent and each has is proper $\cplx$-Gaussian with own precision: $
  \pi(\omega)
...
KL(q \| \pi_{\tau^\ast})
  ...
  \,,
\end{equation}
which exactly replicates \cite{kharitonov_variational_2018}.
