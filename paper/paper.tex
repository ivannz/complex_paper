...
%notation
We first clarify some notation: $M^{\top}$ is the matrix transpose, $\conj{M}$ is elementwise
...
for $A \in \cplx^{n \times m}$ and $b \in \cplx^{n}$
...
\end{equation}
%
The entropy of a $\cplx$-Gaussian distribution coincides with the entropy of
\eqref{eq:cn-paired-real-density}, and in terms of $\Gamma$ and $C$ can be expresed:
...
\end{equation}
%
% The roles of $\Gamma$ and $C$ is more evident in univariate case: here $
%   \Gamma = \sigma^2 \geq 0
...
\begin{equation}  \label{eq:c-gauss-vi}
  ...
  \,,
\end{equation}
and we use analogous independent proper $\cplx$-Gaussian approximation for convolutional
kernels. As for bias terms, we propose to use their MAP estimates, i.e. assume an independent
fully factorized Dirac approximation.
...
We formulate $\cplx$ version of the local reparameterization trick using a dense linear
layer as an example, and afterwards outline the caveats for $\cplx$-convolutions.
...
\begin{equation}  \label{eq:cplx-gauss-trick}
  y_i
    ...
    \,.
\end{equation}
For a proper approximation \eqref{eq:c-gauss-vi} the resulting relation coefficients are zero.

\eqref{eq:cplx-gauss-trick} essentially requires three matrix-vector operations: $\mu x + b$,
$\Sigma \lvert x \rvert^2$ and $C x^2$ with the modulus and power applied elementwise. Similar
to $\real$ case, variational dropout for $\cplx$ convolutions draws independent kernel $W$
for each spatial patch in the input, since a convolution is a matrix product of $W$, unrolled
into a toeplitz matrix, and flattened intput $\vec{x}$. Weight sharing is essentially pushed
to shared variational parameters $\theta$ in the approximation $q_\theta$. The rationale
is to reduce correlation in the gradient estimator stemming from kernel weights being
shared between overlapping patches, \cite{kingma_variational_2015,citation_needed}. Thus
\eqref{eq:cplx-gauss-trick} requires computing $\cplx$ convolutions of $x$, complex square
$x^2$, and amplitude $\lvert x \rvert^2$ with the mean, relation, and variance kernels,
respectively.
%
...
We consider two priors for Bayesian dropout: log-uniform and $\cplx$-Gaussian empirical Bayes,
\cite{kingma_variational_2015,molchanov_variational_2017,kharitonov_variational_2018}. For a 
fully factorized approximation $q(\omega)$ and factorized prior belief $\pi(\omega)$, the
divergence term in ELBO \eqref{eq:elbo} is
...
KL(q\| \pi)
  ...
  \,.
\end{equation}
Note that for $\mu \neq 0$ from \eqref{eq:cn-affine} we have $
  \mathcal{N}^{\cplx}(\mu, \alpha \lvert \mu \rvert^2, 0)
...
$ on $x < 0$, asymptotically $
  \log{(-x)} - \gamma
$ as $x\to 0-$ and $
  \mathop{Ei}(x) \geq -e^x
$ on $x \in (-\infty, -1]$ ($\gamma$ is Euler's constant).
...
%
After cancellation \eqref{eq:var-do-kl-div-raw} becomes
...
KL(q\| \pi)
  ...
  \,.
\end{equation}
% For $\beta = 2$ we add $\gamma$ to make the right-hand side nonegative. (dubious)
Gradient-based methods that optimize \eqref{eq:elbo} usually do not rely on the value
of the objective. Since $\mathop{Ei}$ has simple analytic derivative and downstream
computations depend \eqref{eq:var-do-kl-div} only additively in\eqref{eq:elbo}, it is
possible to backpropagate through the divergence without forward evaluation.
