...
datasets and music transcription on MusicNet \citep{thickstun_learning_2017}. We reliably
replicate the state-of-the-art result for $\cplx$-valued networks on MusicNet with $50-100\times$
compression rate.
\end{abstract}
...
split to gauge the final performance. Same appriach was applied to CIFAR10 (sec.~%
\ref{sub:cifar10_100} experiments, which could partly expalin the overall poor test
perfomrnace we got from the tested VGG16 model ($\real$ and $\cplx$ version).
...
its architectural twin, fit to on the raw color data. At the same time complex-valued
models
