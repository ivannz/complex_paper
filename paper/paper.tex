...
\subsection{Variational Dropout} % (fold)
\label{sub:variational_dropout}
...
In appendix~\ref{sub:real-chisq-grad} we verify the value and the derivative of their
...
capabilities of Variational Dropout, optimizing $\alpha$ for each individual parameter.
\citet{kharitonov_variational_2018} address theoretical issues with improper priors and
propose an alternative Bayesian Dropout method based on \textit{Automatic Relevance
Determination} procedure. In essence, the log-uniform factor $\pi(\omega_{ij})$ is replaced by
a proper ARD Gaussian prior $
...
\begin{equation}  \label{eq:ard-kl-div-real}
  KL(q_\theta \| \pi)
    ...
    \,.
\end{equation}
Flexibility in specification of $q_\theta$ enables developing structured dropout with
shared $\alpha$ across groups of parameters $\omega$. 
...
the stochasticity of $
  W \sim \prod_{ij} \mathcal{N}(\mu_{ij}, \alpha_{ij} \mu_{ij}^2)
$ in a linear layer $
  y = b + W^\top x
$ with $
  W \in \mathbb{R}^{n\times m}
$ is pushed to its output
...
and then to deriving explicit expressions for the divergence penalties in \eqref{eq:elbo}
in terms of \verify{special} functions.
...
estimator stemming from decorrelated gradient estimates from overlapping patches
\citep{kingma_variational_2015}, allowing \eqref{eq:cplx-gauss-trick} to leverage $\cplx$
convolutions of the relation and variance kernels with elementwise complex squares $x^2$
and amplitudes $\lvert x \rvert^2$.
...
dense linear, convolutional and other, effectively, affine layers, i.e. we let $
  \Sigma_{ij} = \alpha_{ij} \lvert \mu_{ij} \rvert^2
$ and fix $C_{ij} = 0$. Under this approximation the relation coefficients in \eqref{eq:cplx-gauss-trick}
are zero.
%
...
dropout sec.~\ref{ssub:vd_prior} (or sec.~\ref{ssub:ard_prior}) or its $\real$
...
weights in each layer based on the relevance scores $\alpha$ (sec.~\ref{sub:variational_dropout}). Since
...
\subsection{MNIST-like experiments} % (fold)
...
% subsection mnist_like_experiments (end)

\clearpage

\subsection{CIFAR10 experiments} % (fold)
...
% subsection cifar_experiments (end)
