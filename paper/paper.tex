...
performance and size. Most popular techniques include weight pruning \citep{citation_needed},
model quantization from IEEE754 floating point to integer-based arithmetic \citep{citation_needed},
...
used towards model sparsification, \citep{kingma_variational_2015,molchanov_variational_2017},
...
by differentiable unbiased Monte-Carlo estimators, \citep{kingma_auto-encoding_2014}.
%
...
$C = 0$. The entropy of proper $\cplx$-Gaussians is
...
efficiency of the SGVB gradient estimator, \citep{wang_fast_2013,kingma_variational_2015}. A
general mean field $\cplx$-Gaussian approximation \eqref{eq:c-gauss-vi} for $
  W \in \cplx^{n\times m}
$ is
...
%       }{n! \Gamma(n + k + 1)}
%   \,. $$
In \eqref{eq:ard-kl-div-real} we need $
...
\tfrac{d}{d \alpha} g_1(\tfrac1{2\alpha})
  ...
  \,. $$
Since $\alpha$ is nonnegative, it is typically parameterized via its logarithm and computed
when needed. Thus the gradient of \eqref{eq:ard-kl-div-real} w.r.t $\log \alpha$ is
...
We replicated the simulation experiment in \citep{molchanov_variational_2017} and recovered
the same nonlinear approximation result reported therein. We sampled $10^7$ draws of  and 
The coefficients are within $1\%$ relative tolerance of the reported.
$$
...
$ as an equivalent in algebraic sense function $F\colon \real^2 \to \cplx$ defined $
  f(z) = f(u + jv) = F(u, v)
$. Within this framework the differential of $f$ at $z \in \cplx$ is
$$
