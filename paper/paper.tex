...
$. The divergence term in \eqref{eq:sgvb_estimator} unravels into the following sum:
...
\eqref{eq:sgvb_estimator} over $\tau$, holding other parameters fixed, yields $
...
% , that for the case of fully factorized Gaussian
% posterior approximations, using \eqref{eq:r-gauss-trick} decorrelates the estimators
% \eqref{eq:sgvb_estimator} within the mini-batch and makes the gradient estimator $
...
% $ by introducing \textit{additive noise parameterization}. 

Another contribution of \citep{kingma_variational_2015} is the analysis of the effects of the
\textit{local reparameterization trick} on the variance of the gradient of \eqref{eq:sgvb_estimator}.
...
of SGVB \eqref{eq:sgvb_estimator}, by rendering the gradient with respect to $\mu$ independent from
