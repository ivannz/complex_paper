...
complex conjugation, and $M^{\hop} = (\conj{M})^\top$ denotes Hermitian conjugate.
...
$, with mean $\mu \in \cplx^m$ and $\cplx^{m\times m}$ covariance and relation
matrices $\Gamma$ and $C$ are, respectively, if 
...
$. This distribution possesses the usual properties of $\real$-valued Gaussians, including
if $y = A z + b$ for $
  A \in \cplx^{n \times m}
$ and $b \in \cplx^{n}$, then $
  y \sim \mathcal{N}_n^{\cplx}\bigl(
    ...
    \bigr)
$. The entropy of a $\cplx$-Gaussian distribution $
  q = \mathcal{N}^{\cplx}_m(\mu, \Gamma, C)
$ is
\begin{align} \label{eq:c-gauss-entropy-derivation}
  \mathbb{H}(q)
    ...
    \,.
\end{align}
A $\cplx$-Gaussian random vector $z$ is \textit{proper}, if $z$ and $\conj{z}$ are uncorrelated,
i.e. $C = 0$, since the matrices $\Gamma$ and $C$ correspond to $
  \mathbb{E} (z - \mu)(z - \mu)^{\hop}
$ and $
  \mathbb{E} (z - \mu)(z - \mu)^{\top}
$, respectively. From \eqref{eq:c-gauss-entropy-derivation} its entropy is therefore 
...
Not every $\real^{2m}$ Gaussian corresponds to a $\cplx^m$ Gaussian vecor,
but when it does the covariance and relation can be recovered through
$$
\Sigma
  = \begin{pmatrix}
    \Sigma_{rr} & \Sigma_{ri} \\ 
...
If $M \in \real^{m\times m}$, $M \succeq 0$ and $M^\top = M$, then
$$
  (u - j v)^\top M (u + j v)
    = u^\top M u + v^\top M v + j (u^\top M v - v^\top M u)
    % = u^\top M u + v^\top M v + j (u^\top M v - u^\top M^\top v)
    = u^\top M u + v^\top M v \geq 0
\,. $$
if $M^\top = - M$, then
$$
  u^\top M v - v^\top M u - j (u^\top M u + v^\top M v)
  % = u^\top M v - u^\top M^\top v - j (u^\top M u + v^\top M^\top v)
  = u^\top M v + u^\top M v - j (u^\top M u - v^\top M v)
  \,. $$
