...
methods tailored to $\cplx$-domain remain a niche field of research.
...
through operations in $\real^2$, \citep{hirose_complex-valued_2009}. Activations, considered
in \citep{trabelsi_deep_2017} include independent componentwise application of $\real$-valued
non-linearities, $
  z = u + j v
...
kernel $W$ for each spatial patch in the input. The rationale is simpler computations and
variance reduction. Reduced variance stems from decorrelated gradient estimates from
...
\begin{equation}  \label{eq:cplx-gauss-trick-appendix}
  y_i
    ...
    \,.
\end{equation}
Hence \eqref{eq:cplx-gauss-trick-appendix} folows.
