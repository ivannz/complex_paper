...
$C = 0$. The entropy of a proper $\cplx$-Gaussian is
...
and $b \in \cplx^n$ the output $y = W x + b$ is distributed like
...
\begin{equation}  \label{eq:c-gauss-vi}
  ...
  W \sim q(W)
    % = \prod_{ij} q(W_{ij})
    = \prod_{ij} \mathcal{N}^{\cplx}(W_{ij} \mid \mu_{ij}, \sigma^2_{ij}, 0)
  \,,
\end{equation}
and we use similar factorized approximation for convolutions. We propose to MAP estimates,
...
%notation
By $e_i$ we denote the $i$-th real unit vector of dimensionality \textit{conforming} to
the matrix-vector expression it is used in. $\vec{M}$ denotes \textit{row-major} flattening
of $M$ into a vector, i.e. in lexicographic order of its indices. $\diag{(\cdot)}$ embeds
$\cplx^n$ into $n\times n$ diagonal matrices. $\otimes$ denotes the Kronecker product,
