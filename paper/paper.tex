...
Local Reparameterization Trick. They reverted the $(\mu, \alpha)$ parameterization back
to the equivalent $(\mu, \sigma^2)$, arguing that it reduces the variance of the SGVB
\eqref{eq:sgvb_estimator}, by rendering the gradient with respect to $\mu_{ij}$ independent
from the local output noise, injected by \eqref{eq:r-gauss-trick}. This improvement is
especially important for model pruning, since $\mu_{ij}$ of relevant parameters are used
as point estimates of their values.

Due to factorization alternative parameterization, the term $KL(q_\theta \| \pi)$ in
\eqref{eq:sgvb_estimator} of Sparse VD unravels into $
...
$\pi$, emphasized by \citet{hron_variational_2018}, and proposed Sparse VD based on
\textit{Automatic Relevance Determination} procedure. ARD replaces $\pi(\omega_j)$ by
a proper Gaussian ARD prior $
  \pi_\lambda(\omega_j) = \mathcal{N}(\omega_j \vert\, 0, \tau_j^{-1})
$ with learnable precision $\tau_j > 0$ \citep{neal_bayesian_1996}, thereby recasting
the VD as the \textit{Empirical Bayes} approach, which performs Bayesian Inference over
$\omega$, but uses Maximum Likelihood point-estimates for the hyper-parameters $\lambda$,
