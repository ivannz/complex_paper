...
Let $W \in\real^{n\times m}$, $b\in \real^n$ and consider the action of a linear layer
on its input $x\in \real^m$: $
  y = W x + b$ for $y\in \real^n
$. The key idea of Bernoulli dropout is to mask elements in the matrix $W$ of the layer
...
of the weights' gradient in the sotchasit approximation of the evidence lower bound. 
...
The key idea of Varaitional Dropout is to assume a Gaussian Mean field variational
approxumation of the posterior distribution of linear layer's weights (or convolutional
kernels, with a caveat\footnote).  \footnotetext{
  at each spatial patch of input data the kenrel is independently redarwn form the
...
In fact mutliplicative Gaussian dropout on weights $W$ is in fact itself a variational
