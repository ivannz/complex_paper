...
complex conjugation, and $M^{\hop} = (\conj{M})^\top$ denotes Hermitian conjugate. By $e_i$
we denote the $i$-th real unit vector of dimensionality \textbf{conforming} to the matrix-vector
expression it is used in. $\vec{M}$ denotes row-major flattening of $M$ into a vector, i.e.
in lexicographic order of its indices. $\otimes$ denotes the Kronecker product, for which we
...
Finally, $
  \diag{}
    \colon \mathbb{K}^n \to \mathbb{K}^{n\times n}
    \colon v \mapsto (\delta_{ij} v_i)_{ij}
$ is a diagonal matrix embedding, $\mathbb{K} = \real$ or $\cplx$.
