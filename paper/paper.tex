...
embedded systems, where it is common to encounter data with natural complex domain
representation. To this end we extend Variational Dropout of \citet{kingma_variational_2015}
and \citet{kharitonov_variational_2018} to complex-valued neural networks building upon
the work of \citet{trabelsi_deep_2017}. We verify the proposed Bayesian technique and
assess the performance-compression trade-off by conducting a large-scale numerical study
of $\cplx$-valued networks on two tasks: image recognition on MNIST-like and CIFAR10
datasets and music transcription on MusicNet. We reliably replicate the state-of-the-art
result by \citet{thickstun_learning_2017} for $\cplx$-valued networks on MusicNet with
$50-100\times$ compression rate.
\end{abstract}
...
develop bayesian dropout for $\cplx$-valued networks.
%
...
\end{equation}
% For $\beta = 2$ we add $\gamma$ to make the right-hand side nonegative. (proof needed)
...
All experiments share the following settings unless specified otherwise:
\begin{itemize}
  \item ADAM optimizer with the base learning rate of $10^{-3}$
  \item learning rate the annealing schedule from \citep{trabelsi_deep_2017}
  \item global gradient clipping within an $\ell_2$-norm ball of radius $0.05$
\end{itemize}
Every experiment is replicated several times to take random effects into account, e.g.
random network initialization prior to ``dense'' stage, randomly ordered batches during
SGD. Throughout every experiment on MNIST-like datasets (sec.~\ref{sub:mnist_like_datasets},
we \textit{fixed} a random subset of the train split of each dataset, and used the test
split to gauge the final performance. Same approach was applied to CIFAR10 (sec.~%
\ref{sub:cifar10_100} experiments, which could partly explain the overall poor test
performance we got from the tested VGG16 model ($\real$ and $\cplx$ version).

\subsection{Fitting and compression} % (fold)
\label{sub:fitting_and_compression}

The goal of the following experiments to study the sparsity-performance trade-off of the
proposed Bayesian sparsification methods for $\cplx$-valued networks. To this end we employ
the following training plan in every experiment. The model is trained in three successive
stages: ``dense'' $\to$ ``sparsify'' $\to$ ``fine-tune''. Parameter values and the internal
state of the optimizer for similar parameters are kept between stages.

At the \textit{``dense''} stage every network is fit ``as-is'', retaining its original
non-bayesian architecture and using only the likelihood from \eqref{eq:elbo} without
variational dropout.
%
...
%
At the \textit{``fine-tune''} stage we deduce sparsity masks from the parameters of the fit
variational approximation and train the remaining non-zero parameters similarly to the
``dense'' stage, i.e. with non-Bayesian layers, but with fixed sparsity masks.
...
$: non-zero if $\log \alpha \leq \tau$, zero otherwise. Figure~\ref{fig:log-relevance}
(top) displays general shape of the distribution of $\log \alpha$ values from the experiment
in sec.~\ref{sub:mnist_like_datasets}, which depicts the typical behaviour noticed in all
experiments conducted on every considered dataset: there always is a single mode within the
moderately negative range, and one or two modes to the right of $0$. The relative mass of
each mode changes with $C$ and the higher the coefficient is, the more mass is shifted to
the positive half. In every experiment the trough between the peaks of the negative mode
and the first positive one is around zero.

\begin{figure}[!ht]
  \centering
  \begin{subfigure}[b]{0.9\textwidth}  % imcl2019-style swears at this
    \centering
    \includegraphics[width=1.\linewidth,draft]{../notebooks/assets/log-alpha__histograms.pdf}
    % \caption{} \label{fig:log-relevance-kde}
  \end{subfigure} \\
  % \hfill
  \begin{subfigure}[b]{0.9\textwidth}  % imcl2019-style swears at this
    \centering
    \includegraphics[width=1.\linewidth,draft]{../notebooks/assets/proba_of_nonzero.pdf}
    % \caption{} \label{fig:log-relevance-proba}
  \end{subfigure}
  \caption{%
    \textit{top}: Estimate of $\log\alpha$ density for a convolutional and dense layer
    from the convolutional model of sec.~\ref{sub:mnist_like_datasets}.
    \textit{bottom}: The probability of a parameter being within $\pm \delta$ relative
    tolerance of its mean.
  }
  \label{fig:log-relevance}
\end{figure}
...
\begin{equation} \label{eq:prob_relevant}
  ...
  \,.
\end{equation}
Figure~\ref{fig:log-relevance} (bottom) shows the probability of a $\real$ (or $\cplx$) 
parameter being non-zero for various $\delta\in(0, \tfrac12)$ as a function of $\log \alpha$.
...
% subsection fitting_and_compression (end)
...
greyscale image datasets to see whether the proposed $\cplx$-variational dropout indeed
sparsifies complex-valued networks. We compare with the studied $\cplx$-valued networks
against their $\real$-valued counterparts both in terms of the resulting sparsity and
test performance. For a fair comparison, we make the $\cplx$-valued network have
approximately half as many parameters, \citep{monning_evaluation_2018}.

Although image data is not $\cplx$ in nature \citet{trabelsi_deep_2017} study the applicability
of $\cplx$-valued networks to image classification (CIFAR-10, CIFAR-100, and a subset of
SVHN). Their approach is to either embed $\real$ in $\cplx$ (by $\Re z = 0$), or train
a smaller auxiliary network, that synthesizes the imaginary component from the raw channel
data. In our experiments we apply the two-dimensional Fourier Transform to get $\cplx$-valued
inputs. We concede that this is a rather unnatural representation of image data, however
the selection of publicly available datasets, which are not artificially $\cplx$-valued,
is limited to audio, speech and music.

Indeed, as is shown by the numerical experiments on MNIST-like datasets and CIFAR10, a
real-valued model trained on the Fourier transformed data underperforms relative to
its architectural twin, fit to on the raw colour data. At the same time complex-valued
models

% a couple of plots from MNIST and Fashion-MNIST
% write a note on the incompressible parameters (bias, batch norm etc.)
% discuss the plots describing how the test-performance grows with compression and then drops if the model is too sparse
% hinting at overfitting/regularization effects of masking and sparsification


% subsection mnist_like_datasets (end)

\subsection{CIFAR10/100} % (fold)
\label{sub:cifar10_100}

% mention the same experimental setup as in MNIST
% describe the VGG16 model and its cplx-valued modification
% mention that fourier-features perfomed worse than raw image data embeded in to $\cplx$
% compare compressed cplx-vgg against real-vgg
% draw some conclusions

% subsection cifar10_100 (end)

\subsection{MusciNet} % (fold)
\label{sub:muscinet}

\citet{trabelsi_deep_2017} designed a complex-valued network that achieved SOTA results
on the music-transcription task on MusicNet, \citep{thickstun_learning_2017}.

% describe musicnet as in Trabelsi et al. (2017) also get some hints from thickstun_learning_2017

% mention 44khz to 11khz downsampling and the extraction of the originally recommended test set
% and the validation set chosen by Trabelsi et al.

% Describe the network by Trabelsi and then present the reslts form then model with kernel size \textbf{6} 
% the causally mantion that you also conducted a large study of an inferior model with kenrel size 3.
% (do not mention that this was a mistake, you dummy!)

The set up of the experiment is thus:
Each full three-stage run is replicated five times take random effects into account, e.g.
random network initialization prior to ``dense'' stage, randomly picked $1000$ batches from
the sample during SGD.

% subsection muscinet (end)
