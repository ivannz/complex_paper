...
% (pending) threshold and fune-tuning? BETTER text
Bottom plot in figure~\ref{fig:hist__and__threshold__tradeoff} show improved performance
for thresholds below $-\tfrac12$ after fine-tuning. For higher $\tau$ the final stage
improves performance of a highly compressed model ($C=0.05$), while leaving moderately
compressed model ($C=0.005$) unaffected. This behaviour can be explained by the regularizing
effect of high compression rates, which servely reduce the excess degrees of freedom,
leaving less capacity for overfitting.
