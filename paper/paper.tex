...
and a tool to update prior beliefs about model's parameters in accordance with evidence or
...
For an observed dataset $
  D = (z_i)_{i=1}^N
$ with parametric statistical model (likelihood) $
  p(D \mid \omega)
    = \prod_i \log p(z_i \mid \omega)
    % $D$ conditional independence given parameters $\omega$
$ the Bayes rule $
  p(\omega \mid D) = \tfrac{p(D \mid \omega) \pi(\omega)}{p(D)}
$ transforms prior hypotheses $\pi(\omega)$ about the unknown distribution of model's
parameters $\omega$ into the posterior distribution $p(\omega \mid D)$.
%
...
to compute numerically. Thus, Variational Inference (VI), an approximate inference method
proposed by \citet{jordan_introduction_1999}, is used to find an approximation within
a parametric distribution family with densities $q_\theta(\omega)$, that is closest
in terms of Kullback-Leibler (KL) divergence to the true posterior distribution, $
...
%
\citet{jordan_introduction_1999} showed that this problem is equivalent to \emph{maximizing}
the \emph{Evidence Lower Bound} (ELBO)
\begin{equation}  \label{eq:elbo_general}
  ...
  \,,
\end{equation}
where $\lambda$ and $\phi$ parameterize the prior and the likelihood, respectively.
%
It is possible to use other objectives, besides \eqref{eq:elbo_general}, provided $
  p(\omega \mid D)
$ is evaluated only through $\log p(D \mid \omega)$ and $\log \pi(\omega)$,
\citep{ranganath_operator_2018}.

To make VI able to handle large-scale datasets \citet{hoffman_stochastic_2013} proposed
\emph{Stochastic Variational Inference}, which uses stochastic gradient optimization over
mini-batches from $D$ to solve \eqref{eq:elbo_general} with exponential families of
distributions. To allow for efficient inference with non-conjugate distributions
in large-scale setting \citet{titsias_doubly_2014} proposed \emph{Doubly Stochastic
Variational Bayes} that uses unbiased Monte-Carlo estimate of the gradient of ELBO
and requires that $q_\theta$ can be efficiently sampled from and have tractable
$\nabla_\theta \log q_\theta(\omega)$.
%
Independently, \citet{kingma_auto-encoding_2014} proposed \emph{Stochastic Gradient
Variational Bayes} estimator for ELBO and its gradient \eqref{eq:elbo_general} with
respect to parameters via an unbiased differentiable Monte-Carlo estimator, provided
the density $q_\theta(\omega)$ is amenable to the \emph{reparameterization trick},
i.e. $
  \omega \sim q_{\theta}
...
$ and $g(\varepsilon; \theta)$ differentiable with respect to $\theta$.
...
% SGVB is specifically for VAE (local latent varaibles per data-point)
% what Kingma propose is SGVB for global shared latent varaibles aka
% WEIGHTS!!
The SGVB objective is
\begin{equation}  \label{eq:sgvb_objective}
  \widetilde{\mathcal{L}} %(\theta; \phi, \lambda)
    = - KL(q_{\theta} \| \pi_{\lambda})
      ...
      + \frac1{L} \sum_{i=1}^N \sum_{l=1}^L
        \log p_{\phi}(z_i \mid g(\varepsilon_{il}; \theta))
...
$ is an iid sample from $p_\varepsilon$. The KL divergence term can replaced by an
unbiased differentiable finite sample estimator if it is intractable.

For the purposes of this study, we follow \citet{kingma_auto-encoding_2014} and use
\eqref{eq:sgvb_objective} with random batches of size $M$ from $D$ and $L=1$ $\varepsilon$
samples per datapoint in the mini-batch:
\begin{equation}  \label{eq:elbo}
  \widetilde{\mathcal{L}} %(\theta; \phi, \lambda)
    ...
    \,,
\end{equation}  % \citet{kingma_auto-encoding_2014}
for a random subsample $(z_{i_k})_{k=1}^M$ and iid $\varepsilon_k \sim p_\varepsilon$.

\medskip
With a special posterior approximation family and suitable prior assumptions, Variational
Inference can be used as model regularization and sparsification technique.
...
with weight-input dependent mean and variance. \citet{srivastava_dropout:_2014} propose
...
% Nonlinearities used in $\cplx$VNN are of two classes: split that operate on equivalent
% planar or polar representations of $\cplx$, of full which consider the complex number
% as a whole. Split planar $
%   z \mapsto g(\Re{z}) + \iu g(\Im{z}) % split
% $, \citep{benvenuto_complex_1992,hirose_generalization_2012,haensch_complex-valued_2010},
% or polar $
%   z \mapsto g(\lvert z\rvert) e^{\iu \arg{\!(z)}}
% $, \citep{wolter_complex_2018,arjovsky_unitary_2016}. Full trigonometric, holomorphic, 
% % \citet{hirose_generalization_2012} polar (tanh)
% % \citet{savitha_2011} trig (sinh)
% % \citet{arjovsky_unitary_2016} polar (modReLU)
% % \citet{guberman_complex_2016} polar (zRelU)
% % \citet{wolter_complex_2018} gating for $\cplx$-RNN
...
\citet{trabelsi_deep_2018}, who outline the essential building blocks for deep $\cplx$-valued
...
\citep{trabelsi_deep_2018,wolter_complex_2018}.
