...
it is possible to confidently achieve $\approx100\times$ compression of a deep $\cplx$VNN
...
``pre-train'' stage, before the beginning and upon termination of fine-tuning.
%
...
appears to be insufficient to move model away from the parameters after the ``pre-train'' stage.
...
\eqref{eq:elbo_with_coef} ARD yields slightly higher performance post fine-tuning at
the cost of marginally lower compression.

The results of this experiment and previous ones indicate that the $\cplx$ Variational
Dropout offers adequate compression with controllable loss in performance in $\cplx$VNN
and has trade-off profile similar to the $\real$ counterpart. Although ARD performs on
par with VD, unlike it, it uses a proper prior and has a tractable analytic expression
for the KL-divergence \eqref{eq:emp-bayes-kl-div} in both $\real$ and $\cplx$ cases.%
%
